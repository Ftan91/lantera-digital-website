...
**Assets:** `src/assets/logo.svg` (full lockup) and `favicon.svg` (icon-only mark) are hand-built SVGs — a faceted lantern glyph with a warm gradient "glow", paired with a navy/amber wordmark. `src/assets/founders/finn.jpeg` and `irfan.jpeg` are the founders' real photos; `.founder-photo` in CSS crops any image to the same circular treatment regardless of the source photo's shape. `src/assets/tech/*.svg` (databricks, anthropic, powerbi, microsoft) are real brand marks pulled from [Simple Icons](https://simpleicons.org) (CC0-licensed icon designs; the underlying trademarks still belong to Databricks/Anthropic/Microsoft respectively) — used only as an informational "technology we work with" display, not implying any formal partnership or certification. The `.tech-logo` chip in CSS is a fixed white background regardless of theme, since these source files are plain black and need a consistent light surface. `src/css/styles.css` defines the design tokens (colors, spacing, type) as CSS custom properties at the top of the file — start there when adjusting the visual style.
